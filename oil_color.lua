...
if core.get_modpath("hades_petz") then
...
if core.get_modpath("hades_animalia") then
...
if core.get_modpath("sculpture") then
...
if core.registered_items[oil_bottle] then
	for color,hexcol in pairs(painting.hexcolors) do
		des_col = color:gsub("^%l", string.upper)
		
		core.register_craftitem("painting:oil_color_"..color, {
		...
		core.register_tool("painting:brush_oil_color_"..color, {
		...
		if feather then
			core.register_tool("painting:feather_oil_color_"..color, {
